						...
						<div class="result-content">
							...
							<c:forEach items="${pageBean.list }" var="n" varStatus="v">
									<tr>
										<td>${v.count }</td>
										<td>
											<a href="${pageContext.request.contextPath }/news_showNews.action?nid=${n.nid}">${n.title }</a>
										</td>
										<td>${n.time }</td>
							...
							<div class="list-page">
								...
								<c:if test="${pageBean.currentPage != 1 }">
										[<A
										href="${pageContext.request.contextPath}/news_findByTime.action?currentPage=${pageBean.currentPage-1}">前一页</A>]
		 						</c:if>

								<c:if test="${pageBean.currentPage!=pageBean.totalPage && pageBean.currentPage<pageBean.totalPage }">
										[<A
										href="${pageContext.request.contextPath}/news_findByTime.action?currentPage=${pageBean.currentPage+1}">后一页</A>] 
								</c:if>
							</div>
						</div>
					</form>
